...
int path_get_filename(const char *fpath, char *buf, int bufmax)
{
	char fname[NAME_MAX];
	path_split(fpath,0,&fname,0,0);
	buf[0] = 0;
	if(strlen(fname)>(size_t)(bufmax-1))
		return -1;
	strcpy(buf,fname);
	return 0;
}
...
void inifile_close(INIFILE_T ini)
{
	iniparser_freedict((dictionary*)ini);
}

INIFILE_T inifile_open(const char *fpath, const char *mode)
{
	return (INIFILE_T)iniparser_load(fpath,mode);
}
int inifile_entry_count(INIFILE_T ini)
{
...
int inifile_get_entry(INIFILE_T ini, int index, char **sect,char **key, char **val)
{
	dictionary* d = (dictionary*)ini;
	if(d==NULL)
		return -1;
...
const char *inifile_getstr(INIFILE_T ini, const char *section, const char *key)
{
	char *buf;
	...
	const char *res = iniparser_getstr((dictionary*)ini, buf);
	delete buf;
	return res;
}
const char *inifile_getstr(INIFILE_T ini, const char *key)
{
	return inifile_getstr(ini,NULL,key);
}
uint32_t inifile_getu32(INIFILE_T ini, const char *section, const char *key)
{
	const char *s = inifile_getstr(ini,section,key);
	if(s==NULL||s[0]==0)
		return 0;
	return str_to_u32(s);
}
uint32_t inifile_getu32(INIFILE_T ini, const char *key)
{
	return inifile_getu32(ini,NULL,key);
}
int32_t inifile_geti32(INIFILE_T ini, const char *section, const char *key)
{
	const char *s = inifile_getstr(ini,section,key);
	return str_to_i32(s);
}
int32_t inifile_geti32(INIFILE_T ini, const char *key)
{
	return inifile_geti32(ini, NULL, key);
}
int inifile_setstr(INIFILE_T ini, const char *section, const char *key, const char *val)
{
	int res;
...
int inifile_setstr(INIFILE_T ini, const char *key, const char *val)
{
	return inifile_setstr(ini,NULL,key,val);
}
int inifile_setu32(INIFILE_T ini,const char *section, const char *key, uint32_t val)
{
	char buf[32];
	snprintf(buf,32,"%u",val);
	return inifile_setstr(ini,section,key,buf);
}
int inifile_setu32(INIFILE_T ini, const char *key, uint32_t val)
{
	return inifile_setu32(ini,NULL,key,val);
}
